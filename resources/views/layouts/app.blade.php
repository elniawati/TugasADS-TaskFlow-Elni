...
<body>
    <nav class="sidebar p-3 d-flex flex-column" id="sidebar">
        ...
        @if(auth()->user()->isAdmin())
            ...
            <a href="{{ route('priorities.index') }}" class="{{ request()->routeIs('priorities.*') ? 'active' : '' }}">
                <i class="bi bi-flag me-2"></i> Prioritas
            </a>
            <a href="{{ route('statuses.index') }}" class="{{ request()->routeIs('statuses.*') ? 'active' : '' }}">
        ...
        <nav class="navbar navbar-light bg-white shadow-sm px-3 navbar-top">
            ...
            <div class="ms-auto d-flex align-items-center gap-3">
                <span class="text-secondary small d-none d-sm-inline">
                    ...
                    <span class="badge bg-primary-subtle text-primary-emphasis ms-1">{{ auth()->user()->role->label ?? '-' }}</span>
                </span>
